...
def OOD_partition_subset_viduser_withheld(perc_withheld,sampled_dataset_folder, init_window,
                                          end_window, videos_train, videos_test, users_train, users_test
                                          train_traces, test_traces):    
    partition = {}
    partition['train'] = []
    partition['test'] = []
    withheld_videos = random.sample(videos_train, perc_withheld*len(videos_train))
    withheld_users = random.sample(users_train, perc_withheld*len(users_train))
    for trace in train_traces:
        user = str(trace[0])
        video = trace[1]
        if (user in withheld_users) or (video in withheld_videos):
    ...
    for trace in test_traces:
        ...
        video = trace[1]
        if (user in withheld_users) or (video in withheld_videos):
            print("User %s or video %s are withheld & in test set" % (user, video))
        # to get the length of the trace
        trace_length = read_sampled_data_for_trace(sampled_dataset_folder, video, user).shape[0]
        for tstap in range(init_window, trace_length - end_window):
            ID = {'video': video, 'user': user, 'time-stamp': tstap}
            partition['test'].append(ID)
    return partition, withheld_videos, withheld_users

def OOD_partition_subset_vid_withheld(perc_withheld,sampled_dataset_folder, init_window,
                                      end_window, videos_train, videos_test,
                                      train_traces, test_traces):    
    partition = {}
    partition['train'] = []
    partition['test'] = []
    ...
    for trace in test_traces:
        ...
        video = trace[1]
        if (video in withheld_videos):
            print("Video %s is withheld & in test set" % (video))
        # to get the length of the trace
        trace_length = read_sampled_data_for_trace(sampled_dataset_folder, video, user).shape[0]
        for tstap in range(init_window, trace_length - end_window):
            ID = {'video': video, 'user': user, 'time-stamp': tstap}
            partition['test'].append(ID)
    return partition, withheld_videos

def OOD_partition_subset_user_withheld(perc_withheld,sampled_dataset_folder, init_window,
                                          end_window, users_train, users_test
                                          train_traces, test_traces):    
    partition = {}
    partition['train'] = []
    partition['test'] = []
    ...
    for trace in test_traces:
        ...
        video = trace[1]
        if (user in withheld_users):
            print("User %s is withheld & in test set" % (user))
        # to get the length of the trace
        trace_length = read_sampled_data_for_trace(sampled_dataset_folder, video, user).shape[0]
        for tstap in range(init_window, trace_length - end_window):
            ID = {'video': video, 'user': user, 'time-stamp': tstap}
            partition['test'].append(ID)
    return partition, withheld_users
...
def format_synthetic_data(sampled_dataset_folder, init_window, end_window):
    partition['test'] = []
    traces = [['37_no_motion', '2_Diner'], ['37_all_random', '2_Diner'],['37_repeating_random','2_Diner']]
    for trace in traces:
        user = = str(trace[0])
        video = trace[1]
        # to get the length of the trace
