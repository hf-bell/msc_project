...
# ----------------------------- TRAIN ----------------------------
def create_TRACK_AblatSal_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT, NUM_TILES_WIDTH):
    ...
    sense_sal_enc_1 = Dense(units=256, name='prop_dense_2_enc1')
    sense_sal_enc_2 = Dense(units=256, name='prop_dense_2_enc2')
    ...
    sense_sal_dec_1 = Dense(units=256, name='prop_dense_2_dec1')
    sense_sal_dec_2 = Dense(units=256, name='prop_dense_2_dec2')
    ...
    out_enc_sal_1 = TimeDistributed(sense_sal_enc_1)(out_flat_enc)
    out_enc_sal_2 = TimeDistributed(sense_sal_enc_2)(out_enc_sal_1)

    conc_out_enc = Concatenate(axis=-1)([out_enc_sal_2, out_enc_pos])
    ...
    for curr_idx in range(H_WINDOW):
        ...
        out_dec_sal_1 = TimeDistributed(sense_sal_dec_1)(flatten_timestep_saliency)
        out_dec_sal_2 = TimeDistributed(sense_sal_dec_2)(out_dec_sal_1)

        conc_out_dec = Concatenate(axis=-1)([out_dec_sal_2, out_dec_pos])
