...
def angle_between_planes(a,b,c):
##    u = normalized(u)
##    v = normalized(v)
    normal_ab = cross(a,b)
    normal_bc = cross(b,c)
    numer = abs(normal_ab[0]*normal_bc[0] + normal_ab[1]*normal_bc[1] + normal_ab[2]*normal_bc[2])
    mag = np.sqrt(normal_ab[0]**2 + normal_ab[1]**2 + normal_ab[2]**2)*np.sqrt(normal_bc[0]**2 + normal_bc[1]**2 + normal_bc[2]**2)
    angle = np.arccos(numer/mag)
    print("angle between planes", radian_to_degrees(angle))
    return angle
...
def compute_rmse(true_position, pred_position):
    mse = mean_squared_error(true_position, pred_position)
    rmse = tf.sqrt(mse)
    return rmse
