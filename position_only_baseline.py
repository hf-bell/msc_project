# https://github.com/yaringal/BayesianRNN/blob/master/Example/sentiment_lstm_regression.py
from tensorflow.keras.models import Model
from tensorflow.keras.layers import Dense, LSTM, Lambda, Input, Reshape, Convolution2D, TimeDistributed, Concatenate, Flatten, ConvLSTM2D, MaxPooling2D, Dropout
...
# ----------------------------- TRAIN ----------------------------
def create_pos_only_model_7(M_WINDOW, H_WINDOW):
...
def create_MC_pos_only_model(M_WINDOW,H_WINDOW):
    # Defining model structure
    encoder_inputs = Input(shape=(M_WINDOW, 2))
    decoder_inputs = Input(shape=(1, 2))
    lstm_layer = LSTM(1024, return_sequences=True, return_state=True, dropout=0.5)
    ...
    # Encoding
    encoder_outputs, state_h, state_c = lstm_layer(encoder_inputs, training=True)
    states = [state_h, state_c]
    ...
    for curr_idx in range(H_WINDOW):
        # # Run the decoder on one timestep
        decoder_pred, state_h, state_c = lstm_layer(inputs, initial_state=states, training=True)
        decoder_pred = dropout_layer(decoder_pred,training=True)
        outputs_delta = decoder_dense_mot(decoder_pred)
        outputs_delta_dir = decoder_dense_dir(decoder_pred)
        outputs_pos = To_Position([inputs, outputs_delta, outputs_delta_dir])
        # Store the current prediction (we will concantenate all predictions later)
