...
#from ATTRACK_model import create_ATTRACK_model, create_ATTRACK_pos_only_model
from CVPR18_model import create_CVPR18_model
import MM18_model
#from position_only_baseline import create_pos_only_model_1, create_pos_only_model_2, create_pos_only_model_3, create_pos_only_model_4, create_pos_only_model_5, create_pos_only_model_6, create_pos_only_model_7, create_pos_only_model_8
from CVPR18_original_model import create_CVPR18_orig_Model, auto_regressive_prediction
#from Xu_CVPR_18.Reading_Dataset import get_videos_train_and_test_from_file
import os
...
parser.add_argument('-tile_layout', action="store", dest='tile_layout', help='ATTRACK tile layout. Format: \"V:H\".')
parser.add_argument('-ds_rate', action="store", dest='ds_rate', help='(Optional) Downsampling factor for saliency maps.')
parser.add_argument('-lr', action="store", dest='lr', help='(Optional) Neural network learning rate.')
parser.add_argument('-beta_c', action="store", dest='beta_c', help='(Optional) Coefficient for KL-loss on c-state distribution.')
parser.add_argument('-beta_h', action="store", dest='beta_h', help='(Optional) Coefficient for KL-loss on h-state distribution.')
...
if args.tile_layout is not None:
    V_TILES, H_TILES = map(int, args.tile_layout.split(':'))

if args.ds_rate is not None:
    DOWNSAMPLE_RATE = int(args.ds_rate)

if args.lr is not None:
    LEARNING_RATE = float(args.lr)
    
if args.beta_c is not None:
    BETA_C = float(args.beta_c)
...
assert model_name in ['ATTRACK', 'TRACK', 'TRACK_VIB', 'CVPR18', 'ATTRACK_pos_only', 'pos_only_1', 'pos_only_2', 'pos_only_3', 'pos_only_4', 'pos_only_5', 'pos_only_6', 'pos_only_7', 'pos_only_8', 'no_motion', 'most_salient_point', 'true_saliency', 'content_based_saliency', 'CVPR18_orig', 'TRACK_AblatSal', 'TRACK_AblatFuse', 'MM18']
...
NUM_TILES_WIDTH = 384 // DOWNSAMPLE_RATE
NUM_TILES_HEIGHT = 216 // DOWNSAMPLE_RATE

NUM_TILES_WIDTH_TRUE_SAL = 256 // DOWNSAMPLE_RATE
NUM_TILES_HEIGHT_TRUE_SAL = 256 // DOWNSAMPLE_RATE

RATE = 0.2

root_dataset_folder = os.path.join('./', dataset_name)
# root_dataset_folder = os.path.join('/home/qguim/PhD/repos/head-motion-prediction/', dataset_name)
...
if model_name == 'ATTRACK':
    full_name = '_'.join([model_name, str(V_TILES), str(H_TILES), str(LEARNING_RATE), str(int(BATCH_SIZE)), str(DOWNSAMPLE_RATE)])
    if args.use_true_saliency:
        RESULTS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Results_EncDec_3DCoords_TrueSal' + EXP_NAME)
        MODELS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Models_EncDec_3DCoords_TrueSal' + EXP_NAME)
    else:
        RESULTS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Results_EncDec_3DCoords_ContSal' + EXP_NAME)
        MODELS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Models_EncDec_3DCoords_ContSal' + EXP_NAME)
if model_name == 'TRACK':
    full_name = '_'.join([model_name, str(LEARNING_RATE), str(int(BATCH_SIZE)), str(DOWNSAMPLE_RATE)])
...
elif model_name in ['pos_only_1', 'pos_only_2', 'pos_only_3', 'pos_only_4', 'pos_only_5', 'pos_only_6', 'pos_only_7', 'pos_only_8']:
    full_name = '_'.join([model_name, str(LEARNING_RATE), str(int(BATCH_SIZE))])
    #full_name = model_name
    RESULTS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Results_EncDec_eulerian' + EXP_NAME)
    MODELS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Models_EncDec_eulerian' + EXP_NAME)
elif model_name == 'ATTRACK_pos_only':
    full_name = '_'.join([model_name, str(LEARNING_RATE), str(int(BATCH_SIZE))])
    RESULTS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Results_EncDec_3DCoords' + EXP_NAME)
    MODELS_FOLDER = os.path.join(root_dataset_folder, full_name + '/Models_EncDec_3DCoords' + EXP_NAME)
...
if model_name not in ['ATTRACK_pos_only', 'pos_only_1', 'pos_only_2', 'pos_only_3', 'pos_only_4', 'pos_only_5', 'pos_only_6', 'pos_only_7', 'pos_only_8', 'no_motion', 'true_saliency', 'content_based_saliency']:
    all_saliencies = {}
    if args.use_true_saliency:
        for video in videos:
            if model_name == 'MM18':
                ...
                arr = load_true_saliency(TRUE_SALIENCY_FOLDER, video)
                all_saliencies[video] = block_reduce(arr, block_size=(1, DOWNSAMPLE_RATE, DOWNSAMPLE_RATE), func=np.mean, cval=np.mean(arr))
    else:
        for video in videos:
            arr = load_saliency(SALIENCY_FOLDER, video)
            all_saliencies[video] = block_reduce(arr, block_size=(1, DOWNSAMPLE_RATE, DOWNSAMPLE_RATE), func=np.mean, cval=np.mean(arr))
...
if model_name == 'content_based_saliency':
    ...
    MODELS_FOLDER = os.path.join(root_dataset_folder, model_name + '_k_' + args.num_of_peaks)
...
def generate_arrays(list_IDs, future_window):
    while True:
        ...
        for ID in list_IDs:
            ...
            x_i = ID['time-stamp']
            # Load the data
            if model_name not in ['ATTRACK_pos_only', 'pos_only_1', 'pos_only_2', 'pos_only_3', 'pos_only_4', 'pos_only_5', 'pos_only_6', 'pos_only_7', 'pos_only_8', 'MM18']:
            ...
            if count == BATCH_SIZE:
                count = 0
                if model_name in ['ATTRACK', 'TRACK', 'TRACK_VIB', 'TRACK_AblatSal', 'TRACK_AblatFuse']:
        ...
        if count != 0:
            if model_name in ['ATTRACK', 'TRACK', 'TRACK_VIB', 'TRACK_AblatSal', 'TRACK_AblatFuse']:
                yield ([np.array(encoder_pos_inputs_for_batch), np.array(encoder_sal_inputs_for_batch),
                        np.array(decoder_pos_inputs_for_batch), np.array(decoder_sal_inputs_for_batch)],
                       np.array(decoder_outputs_for_batch))
            elif model_name == 'CVPR18':
                yield ([np.array(encoder_pos_inputs_for_batch), np.array(decoder_pos_inputs_for_batch), np.array(decoder_sal_inputs_for_batch)], np.array(decoder_outputs_for_batch))
            elif model_name in ['pos_only_1', 'pos_only_2', 'pos_only_3', 'pos_only_4', 'pos_only_5', 'pos_only_6', 'pos_only_7', 'pos_only_8']:
                yield ([transform_batches_cartesian_to_normalized_eulerian(encoder_pos_inputs_for_batch), transform_batches_cartesian_to_normalized_eulerian(decoder_pos_inputs_for_batch)], transform_batches_cartesian_to_normalized_eulerian(decoder_outputs_for_batch))
...
#if model_name == 'DATTRACK':
#    if args.use_true_saliency:
#        model = create_DATTRACK_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT_TRUE_SAL, NUM_TILES_WIDTH_TRUE_SAL, V_TILES, H_TILES, LEARNING_RATE)
#    else:
#        model = create_DATTRACK_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT, NUM_TILES_WIDTH, V_TILES, H_TILES, LEARNING_RATE)
if model_name == 'ATTRACK':
    if args.use_true_saliency:
        models = create_ATTRACK_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT_TRUE_SAL, NUM_TILES_WIDTH_TRUE_SAL, V_TILES, H_TILES, LEARNING_RATE)
    else:
        models = create_ATTRACK_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT, NUM_TILES_WIDTH, V_TILES, H_TILES, LEARNING_RATE)
    model = models['model']
    del models['model']
elif model_name == 'TRACK':
    if args.use_true_saliency:
        model = create_TRACK_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT_TRUE_SAL, NUM_TILES_WIDTH_TRUE_SAL, LEARNING_RATE)
    else:
        model = create_TRACK_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT, NUM_TILES_WIDTH, LEARNING_RATE)
        #model = models['model']
        #embeddings = models['embeddings']
elif model_name == 'TRACK_VIB':
    if args.use_true_saliency:
        model = create_TRACK_VIB_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT_TRUE_SAL, NUM_TILES_WIDTH_TRUE_SAL, LEARNING_RATE)
    else:
        model = create_TRACK_VIB_model(M_WINDOW, H_WINDOW, NUM_TILES_HEIGHT, NUM_TILES_WIDTH, LEARNING_RATE)
...
elif model_name == 'pos_only_1':
    model = create_pos_only_model_1(M_WINDOW, H_WINDOW)
elif model_name == 'pos_only_2':
    model = create_pos_only_model_2(M_WINDOW, H_WINDOW)
elif model_name == 'pos_only_3':
    model = create_pos_only_model_3(M_WINDOW, H_WINDOW)
elif model_name == 'pos_only_4':
    model = create_pos_only_model_4(M_WINDOW, H_WINDOW)
elif model_name == 'pos_only_5':
    model = create_pos_only_model_5(M_WINDOW, H_WINDOW)
elif model_name == 'pos_only_6':
    model = create_pos_only_model_6(M_WINDOW, H_WINDOW)
elif model_name == 'pos_only_7':
    model = create_pos_only_model_7(M_WINDOW, H_WINDOW, BETA_H, BETA_C)
elif model_name == 'pos_only_8':
    model = create_pos_only_model_8(M_WINDOW, H_WINDOW)
elif model_name == 'ATTRACK_pos_only':
    model = create_ATTRACK_pos_only_model(M_WINDOW, H_WINDOW, LEARNING_RATE)
elif model_name == 'CVPR18_orig':
    if args.use_true_saliency:
...
if TRAIN_MODEL:
    ...
    if model_name == 'MM18':
        ...
        model.fit_generator(generator=generate_arrays(partition['train'], future_window=H_WINDOW), verbose=1, steps_per_epoch=steps_per_ep_train, epochs=EPOCHS, callbacks=[csv_logger, model_checkpoint, terminate_on_nan], validation_data=generate_arrays(partition['test'], future_window=H_WINDOW), validation_steps=steps_per_ep_validate)
if EVALUATE_MODEL:
    if model_name not in ['no_motion', 'most_salient_point', 'true_saliency', 'content_based_saliency', 'MM18']:
    ...
    for ID in partition['test']:
        ...
        if video not in outputs_dict.keys():
            errors_per_video[video] = {}
            outputs_dict[video] = {}
            past_traj[video] = {}
            predicted_traj[video] = {}
            true_future_traj[video] = {}
            #fusion_embeddings[video] = {}
        
        if user not in outputs_dict[video].keys():
            outputs_dict[video][user] = {}
            past_traj[video][user] = {}
            predicted_traj[video][user] = {}
            true_future_traj[video][user] = {}
            #fusion_embeddings[video][user] = {}

        try:
            outputs_dict[video][user][x_i] = {k: [] for k in models.keys()}
        except NameError:
            pass

        # Load the data
        if model_name not in ['ATTRACK_pos_only', 'pos_only_1', 'pos_only_2', 'pos_only_3', 'pos_only_4', 'pos_only_5', 'pos_only_6', 'pos_only_7', 'pos_only_8', 'no_motion', 'true_saliency', 'content_based_saliency', 'MM18']:
        ...
        if model_name == 'ATTRACK':
            ...
            for k in models.keys():
                predictions_dict[k] = models[k].predict([np.array(encoder_pos_inputs_for_sample), np.array(encoder_sal_inputs_for_sample), np.array(decoder_pos_inputs_for_sample), np.array(decoder_sal_inputs_for_sample)])
        elif model_name in ['TRACK', 'TRACK_VIB', 'TRACK_AblatSal', 'TRACK_AblatFuse']:
            model_prediction = model.predict([np.array(encoder_pos_inputs_for_sample), np.array(encoder_sal_inputs_for_sample), np.array(decoder_pos_inputs_for_sample), np.array(decoder_sal_inputs_for_sample)])[0]
            #states_fuse = embeddings.predict([np.array(encoder_pos_inputs_for_sample), np.array(encoder_sal_inputs_for_sample), np.array(decoder_pos_inputs_for_sample), np.array(decoder_sal_inputs_for_sample)])
        elif model_name == 'CVPR18':
            ...
            model_prediction = model.predict([encoder_pos_inputs_for_sample, decoder_pos_inputs_for_sample])[0]
            ...
            for t in range(len(groundtruth)):
                ...
                for k, v in predictions_dict.items():
                    outputs_dict[video][user][x_i][k].append(v[t])

            # fde error is the rmse error at the final timestep             
            fde_errors.append(errors_per_timestep[H_WINDOW])
            ade_errors.append(np.mean(errors_per_timestep))
        
        past_traj[video][user][x_i] = np.concatenate([np.array(encoder_pos_inputs_for_sample), np.array(decoder_pos_inputs_for_sample)], axis=-2)
        predicted_traj[video][user][x_i] = model_prediction
        true_future_traj[video][user][x_i] = groundtruth
        #fusion_embeddings[video][user][x_i] = states_fuse
   ...
   # plt.ioff()
    ...
    if test_set:
        avg_orth_error_per_timestep = []
        for t in range(H_WINDOW):
            print('Average', t, np.mean(orth_errors_per_timestep[t]), end=';')
            avg_orth_error_per_timestep.append(np.mean(orth_errors_per_timestep[t]))
            # print t, np.mean(errors_per_timestep[t]), ';'
        print()
        avg_ade_error = np.mean(ade_errors)
        avg_fde_error = np.mean(fde_errors)
        print("AVERAGE ADE ERROR", avg_ade_error)
        print("AVERAGE FDE ERROR", avg_fde_error)
        pickle.dump(orth_errors_per_timestep, open(MODELS_FOLDER + '/orth_err_per_ts.pkl', 'wb'))
        pickle.dump(fde_errors, open(MODELS_FOLDER + '/fde_err.pkl', 'wb'))
        pickle.dump(ade_errors, open(MODELS_FOLDER + '/ade_err.pkl', 'wb'))
        pickle.dump(past_traj, open(MODELS_FOLDER + '/past_traj.pkl', 'wb'))
        pickle.dump(predicted_traj, open(MODELS_FOLDER + '/predicted_traj.pkl', 'wb'))
        pickle.dump(true_future_traj, open(MODELS_FOLDER + '/true_future_traj.pkl', 'wb'))
       ...
       # plt.legend()
       # plt.savefig('evaluate_%s' % model_name)

        pickle.dump(errors_per_timestep, open(MODELS_FOLDER + '/err_per_ts.pkl', 'wb'))
        pickle.dump(errors_per_video, open(MODELS_FOLDER + '/err_per_vid.pkl', 'wb'))
        pickle.dump(outputs_dict, open(MODELS_FOLDER + '/debug_outputs.pkl', 'wb'))
        pickle.dump(past_traj, open(MODELS_FOLDER + '/past_traj.pkl', 'wb'))
        pickle.dump(predicted_traj, open(MODELS_FOLDER + '/predicted_traj.pkl', 'wb'))
        pickle.dump(true_future_traj, open(MODELS_FOLDER + '/true_future_traj.pkl', 'wb'))
        #pickle.dump(fusion_embeddings, open(MODELS_FOLDER + '/embeddings.pkl', 'wb'))
        # print '\n'
